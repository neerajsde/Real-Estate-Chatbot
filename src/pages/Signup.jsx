...
const Signup = () => {
  ...
  useEffect(() => {
    document.title = "Sign up | Real Estate AI";
  }, []);
  ...
  const onSubmit = async (data) => {
    setError("");
    let response = await apiHandler("/users/register", "POST", data);
    if (response.success) {
      toast.success(response.message);
      let loginRes = await apiHandler("/users/login", "POST", data);
      if (loginRes.success) {
        dispatch(fetchUser());
            ...
            <div>
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute bottom-3 right-3 cursor-pointer flex items-center text-gray-500 hover:text-gray-700"
                  tabIndex={
                    -1
